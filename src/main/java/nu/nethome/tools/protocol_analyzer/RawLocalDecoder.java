 ...
 * @author Stefan
 */
public class RawLocalDecoder implements ProtocolDecoder, ProtocolSampler {
    ...
    private boolean isVetting = false;

    public void setTarget(ProtocolDecoderSink sink) {
        decoderSink = sink;
    }

    public RawLocalDecoder() {
        setSampleRate(22000);
    }
    ...
    public int parse(double pulseLength, boolean isMarkPulse) {
        switch (state) {
            case IDLE: {
                if ((pulseLength > 0.0) && (pulseLength < 200000.0) && vet(pulseLength, isMarkPulse) && !isMarkPulse) {
                    if (!isFreeSampling) {
    ...
    private boolean isVetting() {
        return isVetting && (sampleCount == 0);
    }
